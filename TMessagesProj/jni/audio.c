...
JNIEXPORT jint Java_org_konstantin_tyangram_MediaController_startRecord(JNIEnv *env, jclass class, jstring path, jint sampleRate) {
...
JNIEXPORT jint Java_org_konstantin_tyangram_MediaController_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame((uint8_t *) frameBytes, (uint32_t) len, len / 2 < frame_size);
}

JNIEXPORT void Java_org_konstantin_tyangram_MediaController_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}

JNIEXPORT jint Java_org_konstantin_tyangram_MediaController_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT jbyteArray Java_org_konstantin_tyangram_MediaController_getWaveform2(JNIEnv *env, jclass class, jshortArray array, jint length) {
...
JNIEXPORT jbyteArray Java_org_konstantin_tyangram_MediaController_getWaveform(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    jbyteArray result = 0;
...
JNIEXPORT void JNICALL Java_org_konstantin_ui_Stories_recorder_FfmpegAudioWaveformLoader_init(JNIEnv *env, jobject obj, jstring pathJStr, jint count) {
...
JNIEXPORT jboolean Java_org_konstantin_tyangram_MediaController_cropOpusFile(JNIEnv *env, jclass class, jstring src, jstring dst, jlong startMs, jlong endMs) {
...
JNIEXPORT jboolean Java_org_konstantin_tyangram_MediaController_joinOpusFiles(JNIEnv* env, jclass class, jstring file1, jstring file2, jstring dest) {
